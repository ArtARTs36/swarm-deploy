...
## `service_image_update` — Update Service Image Version
**Description**: Updates service image version in stack compose file in push repository, creates branch and commit, pushes branch, and optionally creates merge request.
**Parameters**:
- `stack` (string, required): stack name
- `service` (string, required): service name inside stack compose
- `imageVersion` (string, required): target image tag
- `reason` (string, required): original user prompt
**When to use**:
